...
layui.define(['layer', 'form', 'fsConfig', 'fsButtonCommon'], function(exports) {
	...
	var fsCommon = {
		...
		/**错误msg提示 */
		errorMsg: function(text, callBackFunc) {
			top.layer.msg(text, {
				icon: 2,
				time: 3000
			}, callBackFunc);
		...
		/**成功 msg提示 */
		successMsg: function(text, callBackFunc) {
			top.layer.msg(text, {
				icon: 1,
				time: 3000
			}, callBackFunc);
		...
		//弹出窗口
		open: function(_title, _width, _height, _url, _end, isMaximize, tag) {			
			if($.isEmpty(_width)) {
				_width = "700px";
			}
			if($.isEmpty(_height)) {
				...
				case "upload":
					...
					fsCommon.open(_title, _width, _height, _url, function() {
						...
						if(!$.isEmpty(uploadFilePath)) {
							...
							if(!$.isEmpty(_this.attr("srcElem"))) {
								$(_this.attr("srcElem")).attr("src",uploadFilePath);
							}
						}
					},0,'1');
